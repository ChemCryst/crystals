...
// old syntax
            ...
            case kTPythonScript:
            {
              tokenList.pop_front();  //remove PYSCRIPT token
              // Evaluate in scope of main module
              py::object scope = py::module::import("__main__").attr("__dict__");
              // Evaluate the statements in an separate Python file on disk
              py::eval_file( wxStandardPaths::Get().GetDataDir().ToStdString() + "\py\" + tokenlist[0], scope);
              tokenList.pop_front();  //remove filename token
