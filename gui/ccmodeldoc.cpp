...
CcModelObject *CcModelDoc::FindAtomByLabel(const string &atomname)
{
  ...
  for (list<CcModelAtom>::iterator atom = mAtomList.begin(); atom != mAtomList.end(); atom++)
  {
    if ((*atom).Label() == nAtomname)
      ratom = &(*atom);
  }
  for (list<CcModelSphere>::iterator sphere = mSphereList.begin(); sphere != mSphereList.end(); sphere++)
  {
    if ((*sphere).Label() == nAtomname)
      ratom = &(*sphere);
  }
  for (list<CcModelDonut>::iterator donut = mDonutList.begin(); donut != mDonutList.end(); donut++)
  {
    if ((*donut).Label() == nAtomname)
      ratom = &(*donut);
  }
  m_thread_critical_section.Leave();
  return ratom;
...
CcModelAtom *CcModelDoc::FindAtomByPosn(int posn) // posn is zero based, but id is 1 based.
{
  CcModelAtom *ret = nil;
  m_thread_critical_section.Enter();
  if (posn >= (int)mAtomList.size())
    posn = (int)mAtomList.size();
...
void CcModelDoc::DocToList(CrModList *ml)
{
  ...
  if (!mAtomList.empty())
  {
         ...
         atom++)
    {
      ...
      row.push_back(strm.str());
      strm.str("");
      strm << setprecision(3) << fixed << (*atom).frac_x;
      row.push_back(strm.str());
      strm.str("");
      strm << (*atom).frac_y;
      row.push_back(strm.str());
    ...
    for (list<CcModelSphere>::iterator sphere = mSphereList.begin(); sphere != mSphereList.end(); sphere++)
    {
      ...
      strm.str("");
      strm << (*sphere).m_serial;
      row.push_back(strm.str());
      strm.str("");
      strm << setprecision(3) << fixed << (*sphere).frac_x;
      row.push_back(strm.str());
      strm.str("");
      strm << (*sphere).frac_y;
      row.push_back(strm.str());
      ...
      row.push_back(strm.str());
      strm.str("");
      strm << (*sphere).m_refflag;
      row.push_back(strm.str());
      strm.str("");
      strm << (*sphere).m_assembly;
      row.push_back(strm.str());
      strm.str("");
      strm << (*sphere).m_group;
      row.push_back(strm.str());
      strm.str("");
      strm << (*sphere).m_sflags;
      row.push_back(strm.str());
    ...
    for (list<CcModelDonut>::iterator ring = mDonutList.begin(); ring != mDonutList.end(); ring++)
    {
      ...
      row.push_back(strm.str());
      strm.str("");
      strm << setprecision(3) << fixed << (*ring).frac_x;
      row.push_back(strm.str());
      strm.str("");
      strm << (*ring).frac_y;
      row.push_back(strm.str());
...
CcRect CcModelDoc::FindModel2DExtent(float *mat, CcModelStyle *style)
{
  ...
  for (list<CcModelAtom>::iterator atom = mAtomList.begin(); atom != mAtomList.end(); atom++)
  {
    ...
    if ((style->showres != 0) && ((*atom).m_refflag != style->showres))
      continue;
    if ((style->showgroup != 0) && ((*atom).m_group != 0) && ((*atom).m_group != style->showgroup))
      continue;
    // not excluded
    if (!((*atom).m_excluded))
    {
      ...
      {
        if (c.X() + extra > extent.Right())
          extent.mRight = c.X() + extra;
        if (c.X() - extra < extent.Left())
          extent.mLeft = c.X() - extra;
        if (c.Y() + extra > extent.Bottom())
          extent.mBottom = c.Y() + extra;
        if (c.Y() - extra < extent.Top())
          extent.mTop = c.Y() - extra;
      }
    }
  }
  for (list<CcModelSphere>::iterator sphere = mSphereList.begin(); sphere != mSphereList.end(); sphere++)
  {
    ...
    if ((style->showres != 0) && ((*sphere).m_refflag != style->showres))
      continue;
    if ((style->showgroup != 0) && ((*sphere).m_group != 0) && ((*sphere).m_group != style->showgroup))
      continue;
    // not excluded
  ...
  for (list<CcModelDonut>::iterator donut = mDonutList.begin(); donut != mDonutList.end(); donut++)
  {
    ...
    if ((style->showres != 0) && ((*donut).m_refflag != style->showres))
      continue;
    if ((style->showgroup != 0) && ((*donut).m_group != 0) && ((*donut).m_group != style->showgroup))
      continue;
    // not excluded
    if (!((*donut).m_excluded))
    {
...
int CcModelDoc::FindNextResidueNumber(int current)
{
  ...
  for (list<CcModelAtom>::iterator atom = mAtomList.begin(); atom != mAtomList.end(); ++atom)
  {
    ...
    if (ar > current)
    { // Only need an update if atom res# is > showres, subject to following conditions:
      if (nextres < current)
      { // defo needs updating to something (unless only one residue?)
        nextres = ar;
      }
      else
      { // nextres is bigger than showres. Make it the smallest of nextres and ar.
        if (ar < nextres)
        {
          nextres = ar;
        }
...
int CcModelDoc::FindNextGroupNumber(int current)
{
  ...
  for (list<CcModelAtom>::iterator atom = mAtomList.begin(); atom != mAtomList.end(); ++atom)
  {
    ...
    if (ar > current)
    { // Only need an update if atom group# is > showgroup, subject to following conditions:
      if (nextgroup < current)
      { // defo needs updating to something (unless only one group?)
        nextgroup = ar;
      }
      else
      { // nextgroup is bigger than showgroup. Make it the smallest of nextgroup and ar.
        if (ar < nextgroup)
        {
          nextgroup = ar;
        }
...
bool CcModelDoc::RenderAtoms(CcModelStyle *style, bool feedback)
{
  ...
  if (!(mAtomList.empty() && mSphereList.empty() && mDonutList.empty()))
  {
    ...
    for (list<CcModelAtom>::iterator atom = mAtomList.begin(); atom != mAtomList.end(); ++atom)
    {
      if (((*atom).Label().length() == 0 || (*atom).Label()[0] != 'Q') && VisibleAtomStyle(style, *atom))
...
bool CcModelDoc::VisibleAtomStyle(CcModelStyle *style, CcModelAtom &atom)
{
  ...
  if (
      ...
      ((style->showgroup == 0) || (atom.m_group == 0) || (atom.m_group == style->showgroup)                          // OK if showgroup 0, or if group matches showgroup.
       || ((m_assemblies[atom.m_assembly].size() != 0) && (atom.m_group == *(m_assemblies[atom.m_assembly].begin())) // is the first group of this assembly
...
bool CcModelDoc::RenderBonds(CcModelStyle *style, bool feedback)
{
  ...
  if (!mBondList.empty())
  {
    for (list<CcModelBond>::iterator bond = mBondList.begin(); bond != mBondList.end(); bond++)
    {
      if (!((*bond).m_excluded))
      {
        if (((*bond).m_patms.size() < 2) || (VisibleAtomStyle(style, *((*bond).m_patms[0])) && VisibleAtomStyle(style, *((*bond).m_patms[1]))))
...
string CcModelDoc::FragAsString(const string &atomname, string delimiter)
{
  ...
  for (list<CcModelAtom>::iterator atom = mAtomList.begin(); atom != mAtomList.end(); atom++)
    if ((*atom).spare)
      result += (*atom).Label() + delimiter;
  for (list<CcModelSphere>::iterator sphere = mSphereList.begin(); sphere != mSphereList.end(); sphere++)
    if ((*sphere).spare)
      result += (*sphere).Label() + delimiter;
  for (list<CcModelDonut>::iterator donut = mDonutList.begin(); donut != mDonutList.end(); donut++)
    if ((*donut).spare)
      result += (*donut).Label() + delimiter;
  m_thread_critical_section.Leave();
  return result;
}
...
void CcModelDoc::SendAtoms(int style, bool sendonly)
{
  ...
  for (list<CcModelAtom>::iterator atom = mAtomList.begin(); atom != mAtomList.end(); atom++)
    if ((*atom).IsSelected())
      (*atom).SendAtom(style, sendonly);
  for (list<CcModelSphere>::iterator sphere = mSphereList.begin(); sphere != mSphereList.end(); sphere++)
    if ((*sphere).IsSelected())
      (*sphere).SendAtom(style, sendonly);
  for (list<CcModelDonut>::iterator donut = mDonutList.begin(); donut != mDonutList.end(); donut++)
    if ((*donut).IsSelected())
      (*donut).SendAtom(style, sendonly);
  m_thread_critical_section.Leave();
}
...
void CcModelDoc::ZoomAtoms(bool doZoom)
{
  ...
  if (!mDonutList.empty()) {
    for (list<CcModelDonut>::iterator donut = mDonutList.begin(); donut != mDonutList.end(); donut++)
      if ((*donut).IsSelected())
        (*donut).m_excluded = false;
      else
        (*donut).m_excluded = doZoom;
  }
