...
* Example 1.  All the keywords are given explicitly. The order on the line is
  not important. Note both *MIN* values are given before the *max* values.
...
Commands
^^^^^^^^
Commands to cause CRYSTALS to 'do' something, for example, compute a Fourier
...
issued. They are never more than one line long, and may not have an
...
::
 ...
 Devices used for output
   ...
   PUNCH    - the punch file (bfile#nn.pch by default)
 ...
 Devices used for CRYSTALS own purposes.
   ...
   DISCFILE - the CRYSTALS database (crfilev2.dsc) containing everything for 
   ...
   SPY      - obsolete, used to collect usage statistics on multi-user computers.
...
\\CLOSE devicename 
==================
Any file on the specified device is closed.
...
\\OPEN devicename filename
==========================
This command is similar to RELEASE, except that a wider range of device
names may be specified, and different messages are produced. 
...
List Operations
^^^^^^^^^^^^^^^
Enables the User (or more likely, the programmer) to see which LISTS are being used and 
...
in the *RESULTS* menu exports all lists.
